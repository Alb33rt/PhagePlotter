...
def WeightedReproductionChoice(bacteria_list):
    ...
    candidates = [x for x in candidates if x[1] != None]
...
for sim in range(sims):
    ...
    def DefineGenome(block):
        ...
        for resistance_allele in ["Rif", "Str", "Quin"]:
            ar_locus=random.choice([pos for pos in range(0,len(genome_type)) if ((genome_type[pos][0]!="N") and (not("AR" in genome_type[pos][0])))])
    ...
    for iteration in range(-iterations_outgrowth,iterations):
        ...
        for ant in ant_to_apply:
            ...
            if (param["Environmental Diffusion"]=="Liquid") or (param["Antibiotic Exposure Structured Environments"]=="Homogeneous"):                        
                ...
                spread=2
                random_x=random.sample(list(range(0, param["World Size"])), int(old_div(param["World Size"],spread)))
                random_y=random.sample(list(range(0, param["World Size"])), int(old_div(param["World Size"],spread)))
                   
                for pos_index in range(0, int(old_div(param["World Size"],spread))):
                    antibiotic_pos=(random_x[pos_index],random_y[pos_index])                                      
                    w.world[antibiotic_pos[0], antibiotic_pos[1]].AddAntibiotic(ant,random.choice(list(range(old_div(param["Max Antibiotic Concentration "+ant],2),param["Max Antibiotic Concentration "+ant]))))
