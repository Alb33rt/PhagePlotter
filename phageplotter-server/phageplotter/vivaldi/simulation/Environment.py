        ...
        while current_conc>1:
            ...
            for l in locs:
                if not(l in deployed_in):                
                    self.my_world.world[l].AddAntibiotic(ant_type, int(random.choice(list(range(old_div(current_conc,2), current_conc)))))                
...
class World(object):
        ...
        for i in range(-distance,distance+1,1):
            for j in range(-distance,distance+1,1):
                if i != 0 or j != 0: #Do not add self as neighbor
                    if (origin[0]+i, origin[1]+j) in self.world:
    ...
    def Set_All_Available(self): self.available_locations=list(self.world.keys())
    ...
    def UpdateLocations(self, current_iteration=None):
        ...
        if param["Environmental Diffusion"]=="Liquid":   
            ...
            random_positions=random.sample(list(self.world.keys()), len(list(self.world.keys())))#Randomize locations            
