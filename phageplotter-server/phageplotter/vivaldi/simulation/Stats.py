...
def GetStats(bacteria_list, world):
        ...
        if len(b.phages)>0:            
            ...
            for phg in b.phages:
                if (not(phg["Type"]=="Virulent")) and (not(phg["Type"]=="Induced_Temperate")):
                    ...
                    idxs = [phg["Cargo"][i][3] for i, x in enumerate(list(zip(*phg["Cargo"])[0])) if "AR_Rif" in x]                
    ...
    for loc in world.world:
        for phg in world.world[loc].free_phages:
            ...
            idxs = [phg[1]["Cargo"][i][3] for i, x in enumerate(list(zip(*phg[1]["Cargo"]))[0]) if "AR_Rif" in x]
